...
| Secrets & IAM | **Secret Manager + Workload Identity Federation**, dedicated service accounts (`sa-fastapi`, `sa-streamlit`, `sa-ingest`, etc.) managed via Terraform. | Eliminates long-lived keys, matches security blueprint, tested via dev CI pipeline. | Jerry | 15 Nov 2025 |
