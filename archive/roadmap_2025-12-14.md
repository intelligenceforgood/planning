...
- [x] **Documentation Refresh**: Captured the dev backfill summary, retry procedure, and Vertex quota mitigations across `planning/change_log.md`, `docs/design/architecture.md`, `docs/config/README.md`, and the ingestion runbook so operators know how to throttle or request higher import quotas before the next dataset.
...
- [x] **Diagram Sync**: Migrate dossier architecture flow to Figma/Miro and link from `docs/design/architecture.md`.
...
- [x] **Token Format & Catalog**: `AAA-XXXXXXXX` scheme and prefix registry finalized in [docs/design/pii_vault.md](core/docs/design/pii_vault.md#token-format) with normalization rules.
- [x] **Vault Data Model & Detokenization**: Records, registry, and subpoena workflow defined (KMS-gated, dual-approval, audit + alerting) in [docs/design/pii_vault.md](core/docs/design/pii_vault.md#vault-data-model) and [docs/policies/detokenization_sop.md](core/docs/policies/detokenization_sop.md).
...
- [x] **Smoke**: Vault secret-access verifier and Cloud Run tokenization/detokenization round trip added to [docs/cookbooks/smoke_test.md](core/docs/cookbooks/smoke_test.md#vault-secrets-smoke-gcp); dev run (Dec 11) verified tokenize/detokenize via IAP; prod run still optional before close-out.
