...
- [ ] Create Discovery import manifest referencing uploaded Vertex files. (Not required for Python helper workflow; draft template before next CLI-driven import.)
...
Tasks:
- [x] Identify fields requiring flattening or restructuring to match Discovery `structData` constraints.
...
- 15 Nov 2025: Transformer now sets `structData.source` to the index name when Azure leaves it blank (e.g. intake forms get `source=intake-form-search`), giving us a lightweight dataset tag without touching Discovery schema.
- [x] Determine how to regenerate embeddings (if Azure stored vector fields) using Vertex AI models or existing pipelines.
- MVP decision: rely on Discovery's built-in semantic pipeline; revisit custom embedding generation (Vertex Embedding API vs. repurposing Azure vectors) after stakeholder feedback.

## 4. Import Planning

- [ ] Create dedicated Discovery data stores per index or consolidate into a single store with `structData.index_type` field.
...
- Discovery count check, 14 Nov 2025:
  ```bash
  python - <<'PY'
  from google.cloud import discoveryengine_v1beta as discoveryengine
  ...
  parent = (
    ...
    for doc in client.list_documents(request={"parent": parent}):
      total += 1
    print(f"Total Discovery documents: {total}")
  PY
  ```
  Output 15 Nov 2025: `Total Discovery documents: 1159`. Matches expected (1000 groupsio + 159 intake after dropping 339 attachment-only rows and 7 empty-text rows).
...
- 15 Nov 2025: Added hashed IDs for documents whose Azure IDs exceed 128 chars (Discovery limit). Original ID now saved in `structData.source_id`.
- 15 Nov 2025: Final import run → `success_count=1159`, `failure_count=0`, `total_count=1159` (input set excludes attachment-only and empty-text intake rows). Discovery now holds all non-empty documents.
